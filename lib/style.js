...
Style = (function() {
  ...
  Style.prototype.generate = function(options) {
      ...
      var attr, image, imagePath, images, pixelRatio, relativeImagePath, styles, _i, _len;
      ...
      styles = [this.css(this.selector, ["  background-image: url( '" + relativeImagePath + "' )"])];
	...
	styles.push(this.css('.'+image.selector, attr));
      }
			styles.push("")
      css = styles.join("\n");
	  } else {
			var spriteHeight = options.layout.height / this.pixelRatio
			var spriteWidth = options.layout.width / this.pixelRatio
	    css = " \
